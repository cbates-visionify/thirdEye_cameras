...
def render_2D(left_display, img_scale, objects, is_tracking_on):
    ...
    for obj in objects.object_list:
        if render_object(obj, is_tracking_on):
            ...
            class_and_id = "class: " + str(obj.raw_label) + " ID: " + str(obj.id)
            text_color = (255, 255, 255, 255)
            cv2.putText(left_display, class_and_id, text_position, cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.5, text_color, 1)

            # Diplay Object distance to camera as text
            if np.isfinite(obj.position[2]):
                distance_text = str(round(abs(obj.position[2]), 1)) + "M"
                text_position = (int(position_image[0] - 20), int(position_image[1]))
                cv2.putText(left_display, distance_text, text_position, cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.5, text_color, 1)

            # try:
            #     print('object {} with class {} detected at {} from the camera'.format(str(obj.id), str(obj.raw_label),distance_text))
            # except:
