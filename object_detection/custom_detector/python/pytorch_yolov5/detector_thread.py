...
def main_thread(cam):
    ...
    while not exit_signal:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            # Tracking view - this is the pie chart thing
            track_view_generator.generate_view(objects, cam_w_pose, image_track_ocv, objects.is_tracked)

            cv2.imshow("ZED | 2D View and Birds View ({})".format(cam.serial_number), global_image)
            ...
            if frames >= 100:
                ...
                print('camera {}, fps= {}'.format(cam.serial_number, str(frames / (end_time - start_time))))
                frames = 0
                start_time = time.time()

                # write images to disk
                name = './images/' + cam.serial_number + '/' + cam.serial_number + '_' + str(time.time()) + '.jpg'
                cv2.imwrite(name, global_image)
                print('writing image to disk: ' + name)
...
def main():
    ...
    print('number of cameras detected: {}'.format(len(cameras)))
    for cam in cameras:
        os.mkdir('./images/' + str(cam.serial_number))
        main_cam_thread = Thread(target=main_thread, args=(cam,))
